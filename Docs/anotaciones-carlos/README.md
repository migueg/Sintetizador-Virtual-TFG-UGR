...
que también tiene un tipo de letra distinto. (solucionado)

**Secc. 1.2, página 20 (glosario de términos)**

(2) en varios sitios se dice "es lineal", "no es lineal", "no tiene porque ser lineal". 
En el primer uso de esa palabra habría que explicar que significa "lineal" en este contexto. (solucionado)

(3) "mili segundos" es una única palabra ("milisegundos") (solucionado)

(4) En la definición de "Release" aparace "despear" (solucionado)
...
más abajo. (solucionado)

**Secc 3.1. página 21**

(6) La frase "se dividen en 2 Sprint" debería ser "se dividen en 2 sprints". En general, en el
texto aparece "Sprint", pero creo que se debe usar "sprint" en su lugar. (solucionado)


(7) El segundo párrafo dice que se usa "Scrum", ese párrafo debería ser el primero. (solucionado)
...
está justificado por la derecha. Igualarlo al resto.  (solucionado)


**Secc. 4.1.2, página 26**

(12) Los párrafos son muy cortos y no están separados por líneas. Se usa el acrónimo PH, pero no se entiende muy bien que significa (definirlo en el primer uso).  (solucionado)
...
diagrama. (Las definiciones de DAW y sintetizador ya existen, lo he aclarado un poco más ese parrafo de todas forams) 

**Secc 4.2.2. pág 31**

(14) En el "Escenario 01" se habla del DAW "Ableton Live", pero no se ha introducido antes este 
software concreto. (solucionado)

**Secc 4.3.1 (requisitos funcionales)**

(15) En RF-3.2.2 (pág 35), aparece "20k-0hz", creo que es una errata (debe ser quizás 0-20 Khz, igual que en RF-3.2.3) (solucionado)
...
sería en la sección de objetivos, que ahora mismo se queda corta (ver la nota (5) más arriba) (solucionado)
...
añadir al glosario lo que corresponda.  (Se definen en el mismo requisito)
...
requisito esencial, que debería estar entre los primeros requisitos. (solucionado)
...
mucho más adelante (secc. 6.4.1.3.2.1), pero habría que adelantar la definición al primer uso de la palabra. (solucionado)
...
HU.27 se usa "hacer login" (solucionado)
...
aclarar esto brevemente en el texto. Ver nota (26). (solucionado)


**Secc 5.2**

(21) en el primer párrafo de esta sección se repite "no relaccional"  (solucionado)
...
añadir una etiqueta en el diagrama). (solucionado)
...
al respecto. (se expone después, ya que el análisis es para decidir entre javaScript y c++)
...
usa esas capacidades. Arreglarlo. (solucionado)
...
constructor de la clase `Synth`. (solucionado)
...
6.4.1.3.2.1 (pág. 105). Tmb los nodos envolventes de la 6.4.1.3.2.2. (solucionado)
...
el grafo usado para generar la señal de audio. (solucionado)

**Secc. 6.4.1.3.3. y 6.4.1.3.4 (Reproducción de audio con MIDI)**
...
es en realidad el tiempo de retraso entre la señal original de entrada y la señal retardada de salida (https://developer.mozilla.org/en-US/docs/Web/API/DelayNode/delayTime) (solucionado)
